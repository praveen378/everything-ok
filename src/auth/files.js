...
export class Service {
    ...
    async sendData({ amount, fullName, phone, email, dateOfBirth, pincode }) {
        console.log(amount, fullName, phone, email, dateOfBirth, pincode);
        try {
            return await this.databases.createDocument('userData', 'donation', ID.unique(), {
                amount,       // Save amount
                fullName,     // Save full name
                phone,  // Save phone number
                email,        // Save email
                dateOfBirth,  // Save date of birth
                pincode       // Save pincode
            });
        } catch (error) {
            console.log("Appwrite service :: sendData :: error", error);
        }
    ...
    async createPost({ address, mobile, state, city, userid }) {
        console.log(address, mobile, state, city, userid);
        try {
    ...
    async updatePost({ address, mobile, userid, state, city }) {
        console.log(address, mobile, userid);
        try {
